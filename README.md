...
- поддержки систем отопления (/hvac) [HVAC = Heating, Ventilation, Air Conditioning]
- поддержки систем освещения (/lights)

HVAC endpoint поддерживает как основную функциональность (создание, обновление,
выборку и удаление записей о самой системе), так и места сбора телеметрии с
каждого устройства и отправки управляющего сигнала конкретной системе отопления.
...
Остальные умные устройства предполагается обслуживать по схожей схеме с /hvac и
/lights.